    ...
    implements MavenExecutionRequestPopulator
{
    ...
    public MavenExecutionRequest populateDefaults( MavenExecutionRequest request )
        throws MavenExecutionRequestPopulationException
    {
        // 如果没有指定baseDirectory, 那么使用pom.xml文件作为baseDir赋值给request
        baseDirectory( request );

        // 如果没有指定localRepository,那么使用~/.m2作为默认本地仓库
        // 并且赋值给request
        ...
        // 给request赋值默认的远程仓库: central - https://repo.maven.apache.org/maven2
        injectDefaultRepositories( request );

        // 给request赋值默认的[插件]远程仓库: central - https://repo.maven.apache.org/maven2
